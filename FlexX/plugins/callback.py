...
from FlexX import YouTube, app
from FlexX.core.call import Flex
from FlexX.misc import SUDOERS, db
from FlexX.utils import bot_sys_stats
from FlexX.utils.database import (
    get_active_chats,
    get_lang,
    is_active_chat,
    is_music_playing,
...
from FlexX.utils.formatters import seconds_to_min
from FlexX.utils.inline import (
    stream_markup,
    stream_markup_timer,
    telegram_markup,
...
from FlexX.utils.stream.autoclear import auto_clean
from FlexX.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Flex.pause_stream(chat_id)
        ...
        await Flex.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await CallbackQuery.answer()
        await Flex.stop_stream(chat_id)
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Flex.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                ...
                image = None
            try:
                await Flex.skip_stream(chat_id, link, video=status, image=image)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            if videoid == "telegram":
                ...
                try:
                    ...
                    image = None
            try:
                await Flex.skip_stream(chat_id, queued, video=status, image=image)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
