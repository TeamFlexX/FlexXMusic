...
class Userbot(Client):
    ...
    async def start(self):
        ...
        if config.STRING1:
            ...
            try:
                await self.one.join_chat("XTIORY")
                await self.one.join_chat("Su0FLEX")
            except:
                pass
            assistants.append(1)
            ...
            try:
                await self.one.send_message(
                    config.LOG_GROUP_ID, f"**» تم تشغيل الحساب المساعد : {config.MUSIC_BOT_NAME}**\n\n✨ الايدي : `{self.one.id}`\n❄ الاسم : {self.one.name}\n💫 اليوزر : @{self.one.username}"
                )
            except:
        ...
        if config.STRING2:
            ...
            try:
                await self.two.join_chat("XTIORY")
                await self.two.join_chat("Su0FLEX")
            except:
                pass
            assistants.append(2)
        ...
        if config.STRING3:
            ...
            try:
                await self.three.join_chat("XTIORY")
                await self.three.join_chat("Su0FLEX")
            except:
                pass
            assistants.append(3)
        ...
        if config.STRING4:
            ...
            try:
                await self.four.join_chat("XTIORY")
                await self.four.join_chat("Su0FLEX")
            except:
                pass
            assistants.append(4)
            ...
            try:
                await self.four.send_message(
                    config.LOG_GROUP_ID, f"**» {config.MUSIC_BOT_NAME}  تم تشغيل الحساب المساعد :**\n\n✨ الايدي : `{self.four.id}`\n❄ الاسم : {self.four.name}\n💫 اليوزر : @{self.four.username}"
                )
            except:
                LOGGER(__name__).error(
        ...
        if config.STRING5:
            ...
            try:
                await self.five.join_chat("XTIORY")
                await self.five.join_chat("Su0FLEX")
            except:
                pass
            assistants.append(5)
